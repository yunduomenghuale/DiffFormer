...
def run_training_mlp():
    """
    Main training function for the pbmc68k dataset using the MLP model.
    """
    ...
    config = {
        ...
        "model_type": "MLP",  # Changed model type
        "mlp_params": {
            "time_emb_dim": 32,
    ...
    # Save to a subdirectory to keep MLP results separate
    checkpoints_dir = project_root / "results" / dataset_name / "checkpoints_mlp"
    checkpoints_dir.mkdir(parents=True, exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"--- Starting MLP training for '{dataset_name}' on {device} ---")
    ...
    print(f"MLP Model initialized with {sum(p.numel() for p in model.parameters())} parameters.")
    ...
    print(f"--- MLP Training for '{dataset_name}' complete! ---")
